...
public class LikeController {
    ...
    //like opinion
    @PostMapping("/opinion")
    public ResponseEntity<?> likeOpinion(@AuthenticationPrincipal User user, @PathVariable String opinionID) {
        try {
    ...
    //like comment
    @PostMapping("/comment")
    public ResponseEntity<?> likeComment(@AuthenticationPrincipal User user, @PathVariable String commennID) {
        try {
            LikeDTO likeDTO = LikeDTO.fromLike(likeService.likeOpinion(user, commennID));
            return ResponseEntity.ok(likeDTO);
        }  catch (IllegalAccessException e) {
            return ResponseEntity.status(401).body(e.getMessage());
        } catch (NoSuchElementException e) {
